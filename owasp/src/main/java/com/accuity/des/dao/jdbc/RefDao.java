...
public class RefDao {
	...
	public void addFeedback(Feedback feedback) {
		...
		if (feedback.getName() != null && feedback.getValue() != null) {
			Vector<Object> params = new Vector<Object>();
			params.add(feedback.getName());
			params.add(feedback.getValue());
			
			String sql = " INSERT INTO FEEDBACK " + "(NAME, VALUE) " + " VALUES (?, ?)";	
	...
	public List<Feedback> getFeedback() {
		...
		String sql = "select name, value from feedback where value is not null order by timestamp desc";
		List<Map<String, Object>> rows = jdbcTemplate.queryForList(sql);
		...
		for (Map<String, Object> row : rows) {
			if (row.get("NAME") != null ) {
				...
				feedback.setName(row.get("NAME")+"");
				feedback.setValue(row.get("VALUE")+"");	
	...
	public boolean login(String user, String password) {
		...
		//Defense Option - Escape all input strings
		...
		// Defense Option - Parameterized query
